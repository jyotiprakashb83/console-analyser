...
public class ConsoleParser {
    ...
    final private ConsoleConstantsDisplay displayConstants = new ConsoleConstantsDisplay();

    final private VirtualChannel channel;
    final private boolean preformattedHtml;

    public ConsoleParser(final FilePath parsingRulesFile,
        ...
        statusCount.put(ConsoleConstants.ERROR, 0);
        statusCount.put(ConsoleConstants.WARNING, 0);
        statusCount.put(ConsoleConstants.INFO, 0);

        this.parsingRulesArray = ConsoleUtils
                .readParsingRules(parsingRulesFile);

        // This causes each regular expression to be compiled once for better
        // performance
        this.compiledPatternsPlusError = ConsoleUtils.compilePatterns(
     ...
     */
    @Deprecated
    public ConsoleResult parseLog(final AbstractBuild build) throws IOException, InterruptedException {
        return this.parseLog((Run<?, ?>) build);
    }

    public ConsoleResult parseLog(final Run<?, ?> build) throws IOException, InterruptedException {
        ...
        linkFiles.put(ConsoleConstants.ERROR, errorLinksFilePath);
        linkFiles.put(ConsoleConstants.WARNING, warningLinksFilePath);
        linkFiles.put(ConsoleConstants.INFO, infoLinksFilePath);
        ...
        writers.put(ConsoleConstants.ERROR, new BufferedWriter(new FileWriter(
                errorLinksFilePath)));
        writers.put(ConsoleConstants.WARNING, new BufferedWriter(new FileWriter(
                warningLinksFilePath)));
        writers.put(ConsoleConstants.INFO, new BufferedWriter(new FileWriter(
                infoLinksFilePath)));
        ...
        HtmlWriter.writeHeaderTemplateToAllLinkFiles(writers,
        ...
        writer.write(ConsoleConstants.getHtmlOpeningTags());
        if (this.preformattedHtml)
            writer.write("<pre>");
        // Read bulks of lines, parse
        final int linesInLog = ConsoleUtils.countLines(logFileLocation);
        ...
        if (this.preformattedHtml)
            writer.write("</pre>");
        writer.write(ConsoleConstants.getHtmlClosingTags());
        writer.close(); // Close to unlock and flush to disk.

        ((BufferedWriter) writers.get(ConsoleConstants.ERROR)).close();
        ((BufferedWriter) writers.get(ConsoleConstants.WARNING)).close();
        ((BufferedWriter) writers.get(ConsoleConstants.INFO)).close();

        // Build the reference html from the warnings/errors/info html files
        // created in the loop above
        HtmlWriter.writeReferenceHtml(buildRefPath, headerForSection,
        ...
        HtmlWriter.writeWrapperHtml(buildWrapperPath);

        final String buildUrlPath = build.getUrl(); // job/cat_log/58
        final String buildActionPath = ConsoleAction.getUrlNameStat(); // "parsed_console";
        final String parsedLogURL = buildUrlPath + buildActionPath + "/log.html";

        // Create result class
        final ConsoleResult result = new ConsoleResult();
        result.setHtmlLogFile(parsedFilePath);
        result.setTotalErrors((Integer) statusCount.get(ConsoleConstants.ERROR));
        result.setTotalWarnings((Integer) statusCount
                .get(ConsoleConstants.WARNING));
        result.setTotalInfos((Integer) statusCount.get(ConsoleConstants.INFO));
    ...
    public String parseLine(final String line, final String status)
        ...
        if (status == null) {
            effectiveStatus = ConsoleConstants.NONE;
        } else if (status.equals(ConsoleConstants.START)) {
            effectiveStatus = ConsoleConstants.INFO;
        }
        ...
        if (effectiveStatus != null
                && !effectiveStatus.equals(ConsoleConstants.NONE)) {
    ...
    public void incrementCounterPerSection(final String status,
        ...
        final String key = ConsoleUtils.getSectionCountKey(status,
                sectionNumber);
    ...
    private String addMarkerAndLink(final String line,
        ...
        if (status.equals(ConsoleConstants.START)) {
            sectionCounter++;
            // This enters a line which will later be replaced by the actual
            // header and count for this header
            HtmlWriter.writeHeaderTemplateToAllLinkFiles(writers, sectionCounter); 
    ...
    private void parseLogBody(final Run<?, ?> build, final BufferedWriter writer, final FilePath filePath, final
        ...
        final ConsoleStatus computer = new ConsoleStatus(
